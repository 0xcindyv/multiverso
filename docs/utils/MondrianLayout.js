...
export default class MondrianLayout {
    ...
    fillEmptySpaces(bestSize = true) { // set false to fill with only 1x1 squares
        ...
        for (let square of this.slots) {
            for (let i = 0; i < square.size; i++) {
                for (let j = 0; j < square.size; j++) {
                    if (square.position.y + j < this.height && square.position.x + i < this.width) {
                        occupied[square.position.y + j][square.position.x + i] = true;
                    }
                }
            }
        }
        ...
        const markSquareOccupied = (x, y, size) => {
            for (let i = 0; i < size; i++) {
                for (let j = 0; j < size; j++) {
        ...
        // Copy original slots to the result
        filledSlots = [...this.slots];

        // Fill empty spaces with squares
